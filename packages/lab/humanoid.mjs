...
export function createHumanoid(R,task='reach',routeInput=null){
 const route=routeInput===null?null:readRoute(routeInput,'humanoid');
 if(Boolean(route)!==(task==='custom-route'))throw new TypeError('Custom task and route must be supplied together.');
 if(!route&&!['reach','high-reach'].includes(task))throw new TypeError('Unknown humanoid task.');
 const targets=route?route.points:freeze(task==='reach'?[v(.32,1.15,.45),v(.32,1.45,.5),v(.32,1.65,.35)]:[v(.32,1.4,.35),v(.32,1.7,.35),v(.32,1.6,.5)]);
 ...
 const observe=()=>{const tip=add(lower.translation(),rotate(lower.rotation(),v(0,-H.forearm/2,0)));return freeze({host:'humanoid',profile:H.profile,machineVersion:H.version,task,taskVersion:route?ROUTE_FORMAT:'0.1.0',...(route?{route}:{}),anchored:true,tick,seconds:tick*DT,status,segments:[pose(upper),pose(lower)],tip,velocity:{...lower.velocityAtPoint(tip)},jointAngles:[angle(upper),angle(lower)-angle(upper)],command:command.slice(),goal:targets[Math.min(completed,targets.length-1)],targets,completed,hold,travelMetres:travel});};
