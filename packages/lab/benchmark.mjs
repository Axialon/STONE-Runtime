import {createHumanoid,humanoidDecision} from './humanoid.mjs';
...
/** Fixed reference evaluation only, not a live mutable control session. */
export function benchmark(R,host,task){
 if(host==='rover'){
  ...
  return freeze({host,task,results:rows,recordings:[]});
 }
 if(host!=='humanoid')throw new TypeError('This host has no executable benchmark.');
 const recordings=[],results=[];
 for(const p of packagesFor('humanoid')){
  const h=createHumanoid(R,task),samples=[h.read()];
  try{
   while(h.read().status==='running'){h.step(humanoidDecision(p.id,h.read()));if(h.read().tick%4===0)samples.push(h.read());}
 ...
 return freeze({host,task,results,recordings});
}
export async function digitalBenchmark(R,id,host,task,{cloud=null,consent=false,allowLocalFallback=false}={}){
 ...
 const local={stone:id,version:p.version,execution:p.execution,actualExecution:'local',model:null,host,task,results,text};
