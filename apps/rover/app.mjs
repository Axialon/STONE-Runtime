...
import {makeReplay,MACHINE,ENGINE,PROFILE} from '/rover/contract.mjs';
...
function controls(){
 ...
 routeEditor.setLocked(busy||['running','replay'].includes(phase));text('compare',activeRoute?'Compare this route ↗':'Compare all three ↗');
 text('comparison-scope',activeRoute?'CUSTOM ROUTE / '+activeRoute.name+' · All styles use the same applied checkpoints. This is not the fixed benchmark.':'Identical start, course and machine. Reported values are simulation results, not hardware benchmarks.');
}
function show(sample,index=history.length-1){if(!sample)return;const f=sample.frame,o=f.observation;view.update(sample,history,index);text('time',f.timeSeconds.toFixed(2));text('speed',Math.hypot(o.linearVelocity.x,o.linearVelocity.y,o.linearVelocity.z).toFixed(2));text('height',o.position.y.toFixed(2));text('contacts',o.wheelContacts.filter(Boolean).length+' / 4');text('tick',o.tick);$('scrub').value=String(index);text('route-progress',f.route?'CUSTOM / '+f.route.name+' · Checkpoints '+f.completed+' / '+f.targets.length:'Fixed benchmark / '+course);}
...
async function restart(){
 ...
 try{const s=await client.request('start',{courseId:course,stoneId:selected,...(activeRoute?{route:activeRoute}:{})});if(g!==generation)return;current=s;history=[s];phase='ready';busy=false;document.body.dataset.ready='true';show(s);controls();}catch{if(g===generation)fail('Engine startup failed. Verify the pinned Rapier installation, then Reset.');}
}
...
function stop(){generation++;client?.close();client=null;busy=false;phase='stopped';if(activeRoute&&current?.frame.status==='running'){current={...current,frame:{...current.frame,status:'stopped'}};history[history.length-1]={...history.at(-1),frame:current.frame};show(current);}controls();}
...
$('export').onclick=()=>download('stone-rover-session.json',recording());$('export-results').onclick=()=>download('stone-rover-comparison.json',{engine:ENGINE,course,machineVersion:MACHINE.version,...(activeRoute?{scope:'custom-route',route:activeRoute}:{}),results:comparison});$('passport-export').onclick=()=>download(selected+'-passport.json',getStone(selected));
