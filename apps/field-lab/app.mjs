...
function passport(){const p=getPackage(selected);text('passport-title',p.name);text('passport-limits',p.limits);$('passport').replaceChildren();
...
function cards(){
 ...
 ps.forEach((p,i)=>{const b=document.createElement('button');b.className='stone';b.dataset.package=p.id;
...
function controls(){
 document.body.dataset.host=host;document.body.dataset.phase=phase;document.body.dataset.busy=String(busy);const live=physical(),p=getPackage(selected);
 ...
 $('task').disabled=busy||phase==='running';$('compare').disabled=!live||busy||['loading','running','replay'].includes(phase);
 ...
 document.querySelectorAll('[data-package]').forEach(b=>{const q=getPackage(b.dataset.package),active=q.id===selected;b.setAttribute('aria-pressed',String(active));b.disabled=live&&['loading','replay'].includes(phase);b.querySelector('.state').textContent=q.id===desired&&!active?'Queued':active?(live?'Installed':'Selected'):'Available';});
 $('run-digital').disabled=host!=='digital'||busy||p.execution==='cloud'||(p.execution==='hybrid'&&!$('local-fallback').checked);$('stop-digital').disabled=host!=='digital'||!busy;
}
...
async function restart(){
 ...
 try{const r=await getClient().request('start',{host,task:$('task').value,stoneId:selected});if(g!==generation)return;current=validState(r.state);template=r.recording;history=[current];phase='ready';show(current);view.home();document.body.dataset.ready='true';}
...
function configureDigital(){const p=getPackage(selected);text('digital-title',p.name);text('digital-description',p.description);text('digital-mode',p.execution.toUpperCase());$('fallback-row').hidden=p.execution!=='hybrid';$('cloud-note').hidden=p.execution==='local';
...
async function compare(){const g=generation;busy=true;hideError();controls();try{const r=await getClient().request('compare',{kind:'benchmark',host,task:$('task').value,stoneId:selected,allowLocalFallback:false});if(g!==generation)return;result=r;renderRows(r);}catch{if(g===generation)error('Comparison failed. No new measurements are displayed.');}finally{if(g===generation){busy=false;controls();}}}
async function digital(){const g=generation,[target,task]=$('digital-task').value.split(':');busy=true;clearResult();hideError();text('digital-state','Running local reference tools…');controls();
 try{const r=await getClient().request('compare',{kind:'digital',host:target,task,stoneId:selected,allowLocalFallback:$('local-fallback').checked});if(g!==generation)return;result=r;renderRows(r);text('report',r.text+(r.notice?' — '+r.notice:''));text('digital-state','Completed / '+r.actualExecution+' / no model invoked');}
 catch{if(g===generation){error('The selected Digital Stone could not complete. No cloud result is implied.');text('digital-state','Failed / no new result');client?.close();client=null;}}
 finally{if(g===generation){busy=false;controls();}}
}
function chooseHost(next){if(!['humanoid','drone','digital'].includes(next))return;terminate();host=next;selected=desired=packagesFor(host)[0].id;phase='ready';current=null;history=[];actions=[];template=null;clearResult();hideError();
 document.querySelectorAll('[data-host]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.host===host)));$('humanoid-panel').hidden=!physical();$('digital-panel').hidden=physical();$('planned-panel').hidden=true;
 cards();if(physical()){
 ...
 text('mechanism-title',host==='drone'?'Control in three dimensions.':'Precision in motion.');text('mechanism-label',host==='drone'?'SIMULATED FLIGHT / Q–01':'ARTICULATED REACH / H–01');text('mechanism-scope',host==='drone'?'FOUR ROTORS / SIMULATION ONLY':'ANCHORED TORSO / TWO ACTIVE JOINTS');text('speed-label',host==='drone'?'BODY SPEED':'TIP SPEED');text('hold-label',host==='drone'?'/ 36':'/ 48');text('results-title','Same machine. Different approaches.');void restart();
 }else{configureDigital();controls();}
}
...
$('home').onclick=()=>view.home();$('view3d').onclick=()=>setView('3d');$('viewplan').onclick=()=>setView('plan');$('viewport').addEventListener('rendererchange',()=>setView('plan'));
...
for(const b of document.querySelectorAll('[data-host]'))b.onclick=()=>chooseHost(b.dataset.host);
...
setView('3d');chooseHost('humanoid');raf=requestAnimationFrame(animate);
