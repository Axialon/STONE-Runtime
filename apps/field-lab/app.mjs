...
$('inspection-file').onchange=loadInspectionFile;$('inspection-file').oncancel=cancelInspectionFile;$('inspection-target').onchange=()=>{clearResult();controls();};$('export-manifest').onclick=()=>{try{exportData(manifestFor(selected),'STONE-'+selected+'-manifest.json');}catch{error('Complete model/provider configuration is required; no manifest was invented.');}};
